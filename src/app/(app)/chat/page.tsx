...
function IntegrationNotice({ integration }: { integration: Integration | null }) {
  ...
  const missing = [missingDatafast && "DataFast", missingGithub && "GitHub"]
    .filter(Boolean)
    .join(" & ");

  return (
    <div className="absolute top-4 right-4 z-10 flex items-center gap-2 bg-white border border-amber-200 shadow-sm rounded-xl px-3 py-2 text-xs max-w-xs animate-fade-in">
      ...
      <span className="text-warm-gray">
        Connect <span className="font-semibold text-charcoal">{missing}</span> for live data.{" "}
...
export default function ChatPage() {
  ...
  async function loadData() {
    ...
    const [{ data: msgs }, { data: profile }, { data: intg }] = await Promise.all([
      supabase
        .from("messages")
        .select("*")
        .eq("user_id", user.id)
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto chat-scroll">
        {initialLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center px-6">
            ...
            <h2 className="text-xl font-semibold text-charcoal mb-2">{greeting}</h2>
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto">
          <div className="bg-white rounded-2xl border border-cream-dark/70 shadow-sm overflow-hidden">
            ...
            <div className="flex items-center justify-between px-3 pb-2.5">
              <span className="text-[11px] text-warm-gray/40">Shift+Enter for new line</span>
              <button
                type="submit"
